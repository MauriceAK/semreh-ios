...
final class DirectGoalUITests: XCTestCase {
    ...
    @MainActor
    func testOptInProductionLoginNewChatGoalStatus() async throws {
        continueAfterFailure = false
        #if !targetEnvironment(simulator)
        throw XCTSkip("Direct goal UI verification is simulator-only.")
        #endif
        let environment = ProcessInfo.processInfo.environment
        guard environment["SEMREH_SLICE4_GOAL_UI"] == "1" else {
            throw XCTSkip("Direct goal UI verification is opt-in.")
        }
        guard environment["SEMREH_SLICE2_UI_LIVE"] == "1",
            ...
            return XCTFail("Direct goal UI verification requires the contained pinned stock fixture.")
        }
        ...
        composer.tap()
        composer.typeText("/goal status")
        let send = app.buttons["Send"]
        XCTAssertTrue(send.waitForExistence(timeout: 5))
        send.tap()

        let output = app.staticTexts.matching(
            NSPredicate(format: "label CONTAINS[c] %@", "No active goal. Set one with /goal <text>.")
        ).firstMatch
        let appeared = XCTNSPredicateExpectation(
            predicate: NSPredicate(format: "exists == true AND hittable == true"), object: output
        )
        await fulfillment(of: [appeared], timeout: 60)
        XCTAssertTrue(output.exists && output.isHittable)
        let screenshot = XCTAttachment(screenshot: app.screenshot())
        screenshot.name = "Production goal status in a new chat"
    ...
    private func readCredentials() throws -> Credentials {
        ...
        guard url.resolvingSymlinksInPath().path == credentialsPath else {
            throw NSError(domain: "DirectGoalUITests", code: 1)
        }
        ...
        guard !value.username.isEmpty, !value.password.isEmpty else {
            throw NSError(domain: "DirectGoalUITests", code: 2)
        }
        return value
    }
