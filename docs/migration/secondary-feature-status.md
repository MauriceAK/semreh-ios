...
| Chat skill activation/search | Browsing/search/detail retained. Invocation temporarily deferred; direct dispatch must not be reachable from the shipped UI. | September8 decision under delegated authority, then user approved continuation. Live same-socket fixture dispatch timed out; pinned inline command.dispatch can block reads while waiting for a secret reply on that socket. Track upstream resolution; no private patch or second connection. Authored controller work preserved but not invocation acceptance. Deferral regression/native checks pending. |
| Personality selector | Deferred and hidden for migration; no private route substitution. | Stock config.set persists a profile default and inserts a hidden personality pivot in an attached session, unlike the old session-only promise. Prior history is preserved. Revisit with explicit profile-effect confirmation and readback; no personality mutation run against the fixture. |
| Linked skill files | Deferred and hidden; dead sheet/loader removed. SKILL.md remains supported. | Stock content response has no safe supporting-file read contract; do not turn its absolute path into unrestricted filesystem access. |
