...
Reconciled September 8, 2026 against app `206b216` plus the scoped Git batch:
signed unit-build-v5/UI-build-v3, full-v5 (2235/0/14), UI-v3 (1/0/0). Backend contract pin:
...
| In-chat website login / native credential form | Old `website_login`, `native_component`, `native_component_state` SSE overlays and `/api/native-auth/*`, `/api/work/login/*` controls remain. | Pinned Gateway has no equivalent session-bound event/response/cancel contract. Stock MCP OAuth management is not an equivalent. Recommend temporary defer/hide, awaiting explicit disposition; normal app login and direct approval/clarify/secret/sudo remain supported and must not be removed. |
