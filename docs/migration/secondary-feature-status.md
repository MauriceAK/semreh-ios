...
Reconciled September 8, 2026 against committed app `a758ff6` plus Settings/config
retirement verified by signed build-v1/full-v1 (2265/0/14). Backend contract pin:
...
| Server updates | Still legacy `/api/updates/*`, including the `webui` update target. Not migrated. | Stock check is GET `/api/hermes/update/check?force=...`; POST `/api/hermes/update` starts an action, not confirmed completion. Action status and receipt are not atomically correlated: receipt may belong to an older update. Match a known returned action ID to the durable completion marker; unknown outcomes must not auto-retry or report success. No updater execution against the pinned fixture/personal backend is authorized. |
