...
Reconciled September 8, 2026 against app `2592ee2` plus Kanban/Insights batch:
...
| Goal controls | Retained requirement; direct implementation still open. Maurice approved a bounded two-profile reproduction, not blanket deferral. | Stock dispatch lacks profile binding and quickcommands/plugins precede builtin goal. Original probe failed with an isolation incident, not valid reproduction evidence. Replacement confinement checks failed before stock import; existing owned-gateway alternative under review. No private backend dependency. |
