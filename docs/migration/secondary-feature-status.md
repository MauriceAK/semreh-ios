...
Reconciled September 8, 2026 against app `42c3a48` plus audio/background batch:
signed build-v7/UI-build-v2, full-v2 (2092/0/14), UI-v2 (1/0/0). Backend contract pin:
...
| Goal controls | Still unavailable; no safe direct implementation added. | Stock `command.dispatch` checks quick commands/plugins before builtin `goal` and is not profile-scoped; `command.resolve` does not attest dispatch provenance. Session ID alone does not bind GoalManager's home. Temporary hide/backlog versus upstream release blocker awaits user disposition; no private backend dependency. |
