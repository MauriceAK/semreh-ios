...
| Present reasoning as literal “Thinking ›”; actions retain meaningful icons and truthful labels. | No brain or raw status/ASCII preview in the collapsed reasoning row. Active-only subtle text motion respects Reduce Motion; expandable real details remain available. |
...
| Blank-transcript acceptance requires mixed short/long chat navigation, tab changes, background/foreground, and cold reopen with messages visible before interaction. | Phone feedback reopened this defect after cold-only checks passed. Screenshots after scrolling or accessibility-only observations do not establish resolution. |
